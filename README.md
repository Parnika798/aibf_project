...
- **Natural Language Processing:** Processes the textual content of financial complaints and identifies patterns and information relevant to their classification.
- **Bilingual Complaint Processing:** Supports financial complaints written in **English and Hindi**, enabling the application to work with multilingual customer complaints.
- **Automated Text Classification:** Uses a trained machine learning model to automatically assign a suitable financial category to each complaint.
- **Batch AI Processing:** When a CSV or Excel file containing multiple complaints is uploaded, the AI pipeline processes the complaints in batch and generates a predicted category for every record.
- **Trained AI Pipeline:** The trained classification pipeline is stored in `model/clf_pipeline1.pkl`, while `model/label_encoder1.pkl` converts the model's encoded predictions into meaningful complaint categories.
- **Automated Prediction:** During normal application use, users do not need to manually train the model. The pre-trained AI pipeline is loaded automatically and used for prediction.
- **AI-Driven Analytics:** The predicted categories generated by the AI model are passed to the analytics module to calculate category-wise counts, percentages, and identify the most common complaint categories.
- **Model Evaluation:** During development, multiple machine learning approaches were evaluated, including Logistic Regression, Naive Bayes, Random Forest, LightGBM, Voting Ensemble, and XLM-RoBERTa. The evaluated XLM-RoBERTa model achieved an accuracy of **88.38%**.
